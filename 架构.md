...
1. **User & Memory Service:**
    ...
    - **职责:** (新) 管理 `Users` 和 `Resumes` 表的 CURD。负责用户身份验证和简历库。
        
2. **Interview Orchestration Service:**
    
    - **职责:** 核心“大脑”。处理 `POST /start`，管理 `InterviewSessions` 的状态机（例如：从 `IN_PROGRESS` 到 `COMPLETED`），并协调其他服务。
        
3. **LLM Gateway Service:**
    
    - **职责:** 从 `PromptTemplates` 数据库表中动态获取、缓存和填充 `P-` 系列 Prompt。实现“主题范围上下文”策略，并集中管理对 LLM API 的所有调用。
