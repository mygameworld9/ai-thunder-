...
3. **`InterviewSessions`** (核心的面试会话状态)
    ...
    ```
    {
      ...
      "status": "string_enum", // [CONFIGURING, IN_PROGRESS, COMPLETED, FAILED]
      "difficulty": "string",
...
3. **LLM Gateway Service:**
    ...
    - **职责:** **(更新)** 作为一个核心的**抽象层 (Abstraction Layer)**，统一处理对不同 LLM API (Google, OpenAI, Ollama) 的调用。它会检查当前会话 (`session`) 中存储的 `provider` 和 `model` 字段，然后动态地：
        - 选择对应的 SDK 或 HTTP 客户端。
        - 使用相应的 API Key 或 Base URL 进行认证。
        - 将内部统一的请求格式转换为特定于服务商的格式。
    - 它依然负责从 `PromptTemplates` 数据库表中动态获取、缓存和填充 Prompt，并实现“主题范围上下文”策略。
        
4. **Real-time Service (SSE):**
    
    - **职责:** 管理所有活跃的 SSE 连接。提供一个内部接口（例如通过 Redis Pub/Sub 或内部 HTTP 调用），让 `Orchestration Service` 可以向特定 `session_id` 推送消息。
